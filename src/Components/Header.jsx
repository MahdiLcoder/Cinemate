...
export default function Header() {
  ...
  const [darkMode, setDarkMode] = React.useState(() => JSON.parse(localStorage.getItem("darkMode")) || false); // Dark mode state
    ...
    <header className="sticky top-0 z-50">
      <nav className={`shadow-lg dark:border-gray-600 border-gray-200 ${darkMode ? 'dark:bg-gray-900' : 'bg-white'}`}>
        <div className="flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          {/* Navigation links */}
          <div className={`items-center justify-between ${hidden ? 'hidden' : 'w-full'}  md:flex md:w-auto md:order-1`}>
            <ul className={`flex flex-col p-4 ${hidden ? 'w-full' : ''}  md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700`}>
              <li>
